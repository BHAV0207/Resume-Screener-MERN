...
function UserDashboard() {
  ...
  const notifications = [
    { id: 1, text: "Resume successfully parsed for TechCorp", time: "2 hours ago", type: "success" },
    { id: 2, text: "Application viewed by StartupX hiring team", time: "5 hours ago", type: "info" }
  ];
    ...
    <div className="p-8 ml-8 space-y-10 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8 bg-slate-900 p-10 md:p-12 rounded-[3.5rem] border border-white/5 shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10 space-y-4">
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter uppercase leading-none">Welcome, {user?.name || 'Explorer'} //</h1>
          <p className="text-emerald-500/60 font-black uppercase text-[10px] tracking-[0.4em]">Neural sync active • Candidate protocol initialized</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {stats.map((stat, i) => (
              <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-emerald-900/10 transition-all duration-500 text-center group">
                 <div className={`w-14 h-14 ${stat.bgColor} ${stat.color} rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform`}>
                    <stat.icon size={28} />
                 </div>
                 <div className="text-3xl font-black text-slate-900 tracking-tighter">{stat.value}</div>
                 <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mt-2">{stat.title}</div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm">
             <div className="flex items-center justify-between mb-10">
                <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Recent Pulse</h3>
             ...
             <div className="space-y-6">
                <div className="flex flex-col sm:flex-row items-center justify-between p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:border-emerald-100 transition-colors group">
                   <div className="flex items-center space-x-6">
                      ...
                      <div>
                         <p className="font-black text-slate-900 text-lg tracking-tight uppercase">Tech Lead @ GlobalConnect</p>
                         <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-1">Under Review • Cycle 042</p>
                      </div>
                   </div>
                   <div className="mt-4 sm:mt-0">
                      <span className="px-5 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-900/20">Neural Check</span>
                   </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 opacity-60 group">
                   <div className="flex items-center space-x-6">
                      ...
                      <div>
                         <p className="font-black text-slate-900 text-lg tracking-tight uppercase opacity-50">UI Designer @ PixelPerfect</p>
                         <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-1">Archived • Cycle 038</p>
                      </div>
                   </div>
                   <div className="mt-4 sm:mt-0">
                      <span className="px-5 py-2 bg-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest">Protocol Finalized</span>
                   </div>
                </div>
             </div>
          </div>
        ...
        <div className="space-y-10">
           <div className="bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-2xl shadow-slate-200 overflow-hidden relative group">
              ...
              <h3 className="text-xl font-black mb-6 flex items-center tracking-tight uppercase">
                 <Zap size={24} className="mr-3 text-emerald-500" fill="currentColor" /> Neural Logic
              </h3>
              <p className="text-slate-400 text-sm font-medium leading-relaxed">
                 Optimal matches exceeding <span className="text-emerald-500 font-black">80% efficiency</span> are automatically prioritized in your deployment feed. Sync your node signature for maximum accuracy.
              </p>
           ...
           <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Data Feed</h3>
                <Bell size={24} className="text-emerald-500 animate-swing" />
              </div>
              <div className="space-y-8">
                 {notifications.map(n => (
                   <div key={n.id} className="relative pl-8 border-l-2 border-emerald-100 pb-2 group">
                       ...
                       <p className="text-sm font-bold text-slate-700 leading-snug group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{n.text}</p>
                       <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 mt-2 block opacity-60">{n.time}</span>
                   </div>
                 ))}
              </div>
           </div>
