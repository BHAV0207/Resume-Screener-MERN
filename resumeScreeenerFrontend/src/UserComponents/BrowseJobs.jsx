      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-emerald-500 font-black uppercase text-[10px] tracking-[0.4em] mt-2">
            Neural network active • Discovery protocol active
          </p>
        </div>
      </div>
        ...
        {filteredJobs?.map((job) => (
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em] mb-8">
                {job.company}
              </p>
              ...
              <div className="grid grid-cols-2 gap-6 mb-10">
                ...
                <InfoItem
                  ...
                  text={`${job.minExperience}+ Yrs Exp`}
                />
              ...
              <div className="flex flex-wrap gap-3 mb-10">
                {job.requiredSkills.slice(0, 4).map((skill, i) => (
                  <span
                    key={i}
                    className="px-5 py-2.5 bg-slate-50 text-slate-500 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-100 group-hover:border-emerald-100 transition-colors"
                  >
                    {skill}
                  </span>
                ))}
        ...
        {(!filteredJobs || filteredJobs.length === 0) && (
          <div className="col-span-full py-32 text-center bg-white rounded-[4rem] border border-slate-100">
            ...
            <p className="text-slate-400 font-medium">
              Reset protocol parameters or wait for network expansion.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-3xl flex items-center justify-center z-[100] p-6 animate-in fade-in duration-500">
          <div className="bg-white rounded-[4rem] max-w-2xl w-full p-10 md:p-14 relative shadow-2xl animate-in zoom-in-95 duration-500 overflow-hidden">
            ...
            <button
              ...
              className="absolute top-10 right-10 p-3 rounded-2xl hover:bg-slate-50 text-slate-400 transition-all z-20"
            >
              <X size={28} />
            </button>
            ...
            <div className="text-center mb-12 relative z-10">
              ...
              <p className="text-[10px] font-black uppercase text-emerald-600 tracking-[0.3em]">
                {showApplyModal.title} // {showApplyModal.company}
              </p>
            </div>
            ...
            <div className="space-y-8 relative z-10">
              <div
                className={`relative border-2 border-dashed rounded-[3rem] p-12 text-center transition-all cursor-pointer ${
                  selectedFile
                    ? "bg-emerald-600/5 border-emerald-500 text-emerald-700 shadow-inner"
                    : "bg-slate-50 border-slate-200 hover:bg-white hover:border-emerald-400 group"
                }`}
              ...
              >
                ...
                {selectedFile ? (
                  <div className="flex flex-col items-center">
                    <FileText
                      size={64}
                      className="mb-6 text-emerald-600 animate-pulse"
                    />
                    <span className="font-black truncate w-full px-6 text-lg tracking-tight uppercase">
                      {selectedFile.name}
                    </span>
                    <button
                      onClick={(e) => {
                  ...
                  <div className="flex flex-col items-center">
                    <Upload
                      size={64}
                      className="mb-6 text-slate-200 group-hover:text-emerald-500 transition-all group-hover:scale-110"
                    />
                    <span className="font-black text-slate-400 group-hover:text-slate-900 transition-colors uppercase tracking-widest text-xs">
                      Inject PDF Protocol
                    </span>
                    <span className="text-[9px] font-black uppercase text-slate-300 mt-2 tracking-[0.2em]">
                      MAX BUFFER // 5MB
                    </span>
                  </div>
                )}
