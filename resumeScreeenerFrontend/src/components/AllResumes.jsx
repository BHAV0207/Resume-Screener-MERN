...
const AllResumes = () => {
  const { resumes, fetchAdminResumes } = useContext(JobContext);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Review and manage all candidate applications across the network
          </p>
        </div>
      </div>
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-8">
                <div className="flex items-center space-x-5">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-900 leading-tight group-hover:text-emerald-600 transition-colors uppercase tracking-tight">
                      {resume.name}
                    </h2>
              ...
              <div className="space-y-6 mb-8">
                <div className="flex flex-wrap gap-2">
                  {resume.skills.slice(0, 4).map((skill, index) => (
                    <span
                      key={index}
                      className="px-3 py-1.5 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-wider border border-slate-100 group-hover:border-emerald-100 group-hover:bg-emerald-50 transition-colors"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] border ${getStatusStyle(resume.status)}`}
                  >
                    {resume.status}
                  </span>
                </div>
        ...
        {!filteredResumes?.length && (
          <div className="col-span-full py-32 text-center bg-white rounded-[3rem] border border-slate-100">
            ...
            <p className="text-slate-400 font-black uppercase text-xs tracking-[0.2em]">
              Zero talent nodes detected in current sectors
            </p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-3xl flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[3.5rem] max-w-5xl w-full max-h-[92vh] overflow-hidden flex flex-col relative shadow-2xl animate-in zoom-in-95 duration-500">
            <button
              ...
              className="absolute top-10 right-10 p-3 rounded-full hover:bg-slate-100 text-slate-400 transition-all z-10"
            >
              <X size={32} />
            </button>
            ...
            <div className="p-10 md:p-16 overflow-y-auto custom-scrollbar">
                ...
                <div className="flex items-center space-x-8">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center mt-4 gap-6">
                      ...
                      <span
                        className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border ${getStatusStyle(selectedResume.status)}`}
                      >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="space-y-10">
                  <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2.5 max-h-[300px] overflow-auto">
                      {selectedResume.skills.map((skill, i) => (
                        <span
                          key={i}
                          className="px-4 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-100 shadow-sm"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl shadow-slate-200 relative overflow-hidden group">
                    ...
                    <h3 className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em] mb-4">
                      Maturity Level
                    </h3>
                    <div className="text-5xl font-black tracking-tighter">
                      {selectedResume.experience || 0}{" "}
                      <span className="text-xl text-slate-400">YRS</span>
                    </div>
              ...
              <div className="mt-16 pt-10 border-t border-slate-100 flex flex-wrap gap-6 justify-between items-center">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Sequence ID: {selectedResume._id}
                </p>
                <div className="flex gap-4">
                  ...
                  <button
                    onClick={() => {
                      window.location.href = `mailto:${selectedResume.email}?subject=Regarding your job application&body=Hello ${selectedResume.name},%0D%0A%0D%0AWe would like to discuss your application further.`;
                    }}
                    className="px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase text-xs tracking-[0.2em] hover:bg-emerald-500 hover:-translate-y-1 transition-all shadow-2xl shadow-emerald-900/20 flex items-center"
                  >
